 ...
 * as-is: `botbuilder` and friends are resolved from the host's own
 * `node_modules` at runtime, exactly as every previously published
 * `@omadia/channel-teams` release did. Bundling here would change dependency
 * resolution for a connector that has been installed in production for months,
 * which is not something a packaging script should decide.
 *
 * ## Archive layout: FLAT, on purpose
 *
 * The archive root holds `manifest.yaml`, `package.json` and `dist/` directly —
 * no wrapping `<id>-<version>-package/` directory. This matches every
 * `@omadia/channel-teams` ZIP published so far (verified against the live
 * 0.12.7 artifact). The Telegram script nests its payload one level deeper; the
 * two shapes are not interchangeable, so this one is pinned to what the hub has
 * always received from this connector.
 */
